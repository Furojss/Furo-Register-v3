...
let memberAylar = moment(member.user.createdAt).format("MM").replace("01", "Ocak").replace("02", "Şubat").replace("03", "Mart").replace("04", "Nisan").replace("05", "Mayıs").replace("06", "Haziran").replace("07", "Temmuz").replace("08", "Ağustos").replace("09", "Eylül").replace("10", "Ekim").replace("11", "Kasım").replace("12", "Aralık"); 
    ...
    if (member.user.bot) {
        ...
        let durum = Date.now()-member.user.createdTimestamp < 1000 * 60 * 60 * 24 * 7
              ...
              <a:gzg:829299711965003776> Hoşgeldin ${member} seninle **${member.guild.memberCount}** kişiyiz!\n\n<a:gzg:829299711965003776> ${kayitSorumlusu} rolündeki yetkililer seninle ilgilenecektir.\n\n <a:gzg:829299711965003776> Sunucumuza kayıt olmak için sese girip teyit vermen gerekmektedir.\n\n <a:gzg:829299711965003776> Hesabın Oluşturulma Tarihi: ${memberGün} ${memberAylar} ${memberTarih}\n\n <a:gzg:829299711965003776> Bu Hesap **${durum ? "Şüpheli" : "Güvenli"}**
              `);
